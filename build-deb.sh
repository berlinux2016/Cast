#!/bin/bash
#
# Build Debian package for gnome-cast-player
...
if [ ! -f "videoplayer.py" ]; then
    ...
    echo "Please run this script from the gnome-cast-player directory."
    exit 1
fi
...
rm -rf debian/gnome-cast-player
rm -f ../gnome-cast-player_*.deb
rm -f ../gnome-cast-player_*.build
rm -f ../gnome-cast-player_*.buildinfo
rm -f ../gnome-cast-player_*.changes
rm -f ../gnome-cast-player_*.tar.xz

# Compile locale files
echo -e "${YELLOW}Compiling locale files...${NC}"
if [ -f "locale/de/LC_MESSAGES/gnome-cast-player.po" ]; then
...
if [ -f "locale/en/LC_MESSAGES/gnome-cast-player.po" ]; then
...
if [ $? -eq 0 ]; then
    ...
    ls -lh ../gnome-cast-player_*.deb
    echo ""
    echo "To install the package, run:"
    echo -e "${YELLOW}  sudo dpkg -i ../gnome-cast-player_*.deb${NC}"
