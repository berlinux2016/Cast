...
- `locale/de/LC_MESSAGES/gnome-cast-player.po` (18K) - German
- `locale/en/LC_MESSAGES/gnome-cast-player.po` (12K) - English
...
1. **Button labels** (lines 2931-3903):
   ...
   scan_button.set_tooltip_text(_("Search Cast device Devices"))
   ```
...
4. **Section labels** (lines 3122-3362):
   ...
   label = Gtk.Label(label=_("Cast device Devices"))
   ```
...
_("Search Cast device Devices")           # Cast Geräte suchen
...
sed -i 's/label="Cast Geräte"/label=_("Cast device Devices")/g' videoplayer.py
