...
### Modern GTK4 Video Player with Cast device Streaming and Hardware Acceleration
...
[![GitHub](https://img.shields.io/badge/GitHub-berlinux2016%2Fgnome--chromecast--player-blue?logo=github)](https://github.com/berlinux2016/gnome-cast-player)

**Cross-Distribution Linux Video Player** | **Multilingual (EN/DE)** | **Hardware Accelerated**

*Developed by **BerLinuX** with ❤️ for Simone*
...
![Cast device Mode](screenshots/chromecast-mode.png)
*Cast device streaming with device selection*
-->
...
- **Volume Control** with slider for local and Cast device playback
...
- **Video Streaming** to all Cast device devices
...
- **Subtitle Support** - Display subtitles on Cast device device
- **Audio Track Selection** - Select audio tracks for Cast device playback
- **Multi-Room Audio** - Synchronized playback on multiple devices
- **Group Support** - Connection with Cast device groups
- **Extended Status Display** - Detailed Cast device information in real-time
...
Video Cast Player is designed to work on **all major Linux distributions**:
...
```bash
yay -S gnome-cast-player
# or
paru -S gnome-cast-player
```
...
# Install the package
sudo dnf install ./gnome-cast-player-*.rpm
```
...
- Clean uninstallation via `sudo dnf remove gnome-cast-player`
...
# Install the package
sudo dpkg -i ../gnome-cast-player_*.deb
...
Or search for "Video Cast Player" in your GNOME applications.
...
- Cached thumbnails are stored in `~/.cache/gnome-cast-player/thumbnails/`
...
7. Works for both local playback and Cast device streaming
...
6. **Note**: Feature is only available in local mode, not for Cast device playback.

### Using Chromecast

1. Make sure your Cast device and your computer are on the same network
2. Click the Wi-Fi icon in the header bar to search for Cast device devices
...
8. Converted videos are cached in `~/.cache/video-cast-player/` for faster access next time

### Advanced Cast device Features
...
**Cast device Groups (Multi-Room Audio):**
1. Create groups in the **Google Home App** on your smartphone
2. Add multiple Cast device devices to a group
...
- **Chromecast**: Video is streamed to the connected Cast device device
...
- Python dependencies
- Cast device devices on network
- HTTP server ports

### Automatically Configure Firewall

If Cast device problems occur, run:
```bash
./fix-firewall.sh
```
...
**Symptom**: When clicking "Search Cast device Devices", no devices are displayed.

**Solutions**:
1. Check if your computer and Cast device are on the same Wi-Fi
...
3. Restart the Cast device (unplug and plug back in)
...
### Cast device Streaming Not Working

**Symptom**: Connection to Cast device works, but video is not played.
...
5. **Network Problems**
   - Make sure computer and Cast device are on the same subnet
...
- `pychromecast>=13.0.0` - Cast device control
...
**DaHool** - [GitHub](https://github.com/berlinux2016/gnome-cast-player)
...
1. Cast device streaming requires videos to be accessible via HTTP
...
### 🔧 Cast device Extensions
- [x] **Cast device Subtitles** - Subtitle control on remote device ✓
- [x] **Cast device Audio Tracks** - Audio track selection for Cast device ✓
- [x] **Multi-Room Audio** - Synchronized playback on multiple devices ✓
- [x] **Cast device Groups** - Support for audio groups ✓
- [x] **Extended Status Display** - Detailed Cast device information ✓
...
- 💾 Storage in `~/.config/video-cast-player/recent_files.json`
...
- 💾 Intelligent caching system in `~/.cache/gnome-cast-player/thumbnails/`
...
- ✨ **Cast device Subtitles** - Subtitle support for Cast device playback (VTT format)
- ✨ **Cast device Audio Tracks** - Selection of audio tracks on remote device
- ✨ **Multi-Room Audio** - Synchronized playback on multiple Cast device devices
- ✨ **Cast device Groups** - Automatic detection and connection with groups
...
- 📊 Progress display in percent for Cast device playback
...
- 🎨 Automatic UI updates every 250ms in Cast device mode
...
- 📺 Support for local and Cast device playback of YouTube content
...
- ✨ Timeline/seek function for local and Cast device playback
- ✨ Volume control with slider for local and Cast device playback
- ✨ NVIDIA hardware acceleration (NVDEC/NVENC)
- 🐛 Improved Cast device compatibility (Xiaomi TVs)
- ⚡ Cast device device search 30x faster (500ms instead of 15s)
- 🔧 Mode switching between local and Cast device optimized
...
- ✨ GTK4/Libadwaita UI
- ✨ Cast device streaming with HTTP server
