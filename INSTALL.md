# Installation Guide - GNOME Cast Player

This guide explains the different installation methods for GNOME Cast Player.
...
3. **Uninstall:**
   ...
   ```bash
   sudo dnf remove gnome-cast-player
   ```
...
1. **From application menu:**
   - Search for "GNOME Cast Player" in your application launcher
...
3. **Open a video file directly:**
   ```bash
   gnome-cast-player /path/to/video.mp4
   ```
...
```bash
rm -rf ~/.local/share/gnome-cast-player
rm -f ~/.local/bin/gnome-cast-player
rm -f ~/.local/share/applications/gnome-cast-player.desktop
```
...
Configuration files are stored in `~/.config/video-cast-player`:
...
```bash
make -C ~/rpmbuild/SOURCES gnome-cast-player-2.0.0.tar.gz
```
