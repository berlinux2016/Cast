...
Version:        2.0.1
Release:        1%{?dist}
Summary:        Modern GTK4 video player with Cast device support
...
%description
A modern GTK4/Libadwaita video player with Cast device streaming support.
...
install -m 644 gnome-cast-player.desktop %{buildroot}%{_datadir}/applications/

# Install icon (if exists)
if [ -f gnome-cast-player.svg ]; then
    install -m 644 gnome-cast-player.svg %{buildroot}%{_datadir}/icons/hicolor/scalable/apps/
fi

# Install AppStream metadata (if exists)
if [ -f gnome-cast-player.appdata.xml ]; then
    install -m 644 gnome-cast-player.appdata.xml %{buildroot}%{_datadir}/metainfo/
fi
...
- Fixed application ID to org.gnome.Cast devicePlayer for proper GNOME integration
- Fixed desktop icon name to gnome-cast-player
