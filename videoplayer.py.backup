#!/usr/bin/env python3
"""
GTK4 Videoplayer mit Cast Device Support und AMD Hardware-Beschleunigung (VA-API)
"""
...
class ConfigManager:
    ...
    def __init__(self):
        self.config_dir = Path.home() / ".config" / "video-cast-player"
...
class BookmarkManager:
    ...
    def __init__(self):
        self.bookmarks_dir = Path.home() / ".config" / "video-cast-player"
...
class RecentFilesManager:
    ...
    def __init__(self, max_items=10):
        self.recent_files_dir = Path.home() / ".config" / "video-cast-player"
...
class VideoConverter:
    """Automatische Video-Konvertierung für Cast Device Compatibility"""

    def __init__(self):
        self.conversion_cache_dir = Path.home() / ".cache" / "video-cast-player"
    ...
    def convert_to_mp4(self, input_path, progress_callback=None):
        ...
        try:
            ...
            cmd = [
                ...
                '-c:a', 'aac',   # Konvertiere Audio zu AAC (Cast devices require AAC)
...
class VideoHTTPServer:
    """Einfacher HTTP-Server für Video Streaming to Cast Devices"""
        ...
        class RangeRequestHandler(SimpleHTTPRequestHandler):
            ...
            def handle(self):
                ...
                try:
                    ...
                    # Cast device disconnected (z.B. Stop gedrückt)
...
class CastDeviceManager:
    """Manages Cast Devices und Streaming"""
    ...
    def discover_chromecasts(self, callback):
        """Sucht nach available Cast devices"""
        if self._discovery_browser:
            print("ℹ Cast device discovery läuft bereits.")
            # Sofort die bereits gefundenen Geräte zurückgeben
            callback(list(self._found_devices.values()))
            return

        print("=== Starte asynchrone Cast device discovery ===")
    ...
    def play_video(self, video_path, video_url):
        """Spielt Video on Cast device ab"""
        if not self.selected_cast:
            print("✗ No Cast device ausgewählt")
            return False
        ...
        try:
            ...
            if self.selected_cast.app_id and self.selected_cast.app_id != 'CC1AD845':
                ...
                print("  Beende aktive App for Cast streaming...")
                try:
            ...
            print("  Warte on Cast device-Antwort...")
            ...
            print("  Möglicherweise ist der TV ausgeschaltet or Cast device not responding nicht.")
            return False
        except Exception as e:
            ...
            print("  3. Netzwerkprobleme zwischen computer and Cast device")
    ...
    def pause(self):
        """Pausiert Cast playback"""
        if self.mc:
            try:
                print("Cast: Pause wird gesendet...")
                self.mc.pause()
                print("Cast: Pause-Befehl erfolgreich gesendet")
            except Exception as e:
                print(f"Cast device pause error: {e}")
                import traceback
                traceback.print_exc()
        else:
            print("WARNUNG: Cast Media Controller (self.mc) ist nicht initialisiert!")

    def play(self):
        """Startet/Fortsetzt Cast playback"""
        if self.mc:
            try:
                print("Cast: Play wird gesendet...")
                self.mc.play()
                print("Cast: Play-Befehl erfolgreich gesendet")
            except Exception as e:
                print(f"Cast device play error: {e}")
                import traceback
                traceback.print_exc()
        else:
            print("WARNUNG: Cast Media Controller (self.mc) ist nicht initialisiert!")
    ...
    def get_position(self):
        """Gibt Cast device position in Sekunden zurück"""
        if self.mc and self.mc.status:
            return self.mc.status.current_time or 0.0
        return 0.0

    def get_duration(self):
        """Gibt Cast device duration in Sekunden zurück"""
        if self.mc and self.mc.status:
            return self.mc.status.duration or 0.0
        return 0.0

    def seek(self, position_seconds):
        """Springt zu Position on Cast device"""
        if self.mc and self.selected_cast:
            try:
                print(f"Cast: Seeking to {position_seconds:.1f}s")
                # Warte kurz on Cast device-Bereitschaft
                ...
                print(f"✓ Cast: Seek erfolgreich zu {position_seconds:.1f}s")
            except Exception as e:
                print(f"✗ Cast device seek error: {e}")
                import traceback
                traceback.print_exc()

    def update_status(self):
        """Aktualisiert Cast device status (nötig für Position-Abfragen)"""
    ...
    def set_volume(self, volume):
        """Setzt Cast device volume (0.0 bis 1.0)"""
        if self.selected_cast:
            try:
                if 0.0 <= volume <= 1.0:
                    self.selected_cast.set_volume(volume)
                    print(f"Cast device volume: {volume * 100:.0f}%")
                else:
                    print(f"✗ Ungültiger Lautstärke-Wert: {volume} (muss zwischen 0.0 und 1.0 sein)")
            except Exception as e:
                print(f"✗ Fehler beim Setzen der Cast device volume: {e}")

    def get_volume(self):
        """Gibt aktuelle Cast device volume zurück (0.0 bis 1.0)"""
        if self.selected_cast and self.selected_cast.status:
            try:
                return self.selected_cast.status.volume_level
            except Exception as e:
                print(f"✗ Fehler beim Abrufen der Cast device volume: {e}")
    ...
    def enable_subtitles(self, subtitle_url, subtitle_lang='en-US', subtitle_name='Subtitles'):
        """Aktiviert Untertitel für Cast playback
        ...
        if not self.mc or not self.mc.status:
            print("✗ Kein active Cast stream")
            return False

        try:
            print(f"\n=== Enable Cast subtitles ===")
    ...
    def disable_subtitles(self):
        """Deaktiviert subtitles for Cast"""
        if self.mc:
            try:
    ...
    def set_audio_track(self, track_id):
        """Wählt eine audio track for Cast aus
        ...
        try:
            print(f"Wähle Cast audio track {track_id}")
            # Nutze die native Cast function zum Wechseln der Audio-Spur
    ...
    def get_extended_status(self):
        """Gibt erweiterte Cast device status-Informationen zurück
    ...
    def discover_cast_groups(self):
        """Entdeckt Cast groups für Multi-Room-Audio
        ...
        try:
            ...
            if not groups:
                print("ℹ Keine Cast groups gefunden")
    ...
    def connect_to_group(self, group_service):
        """Verbindet mit einer Cast group für Multi-Room-Audio
...
class VideoPlayer(Gtk.Box):
    ...
    def get_thumbnail_path(self, video_path):
        ...
        # Cache-Verzeichnis
        cache_dir = os.path.expanduser("~/.cache/gnome-cast-player/thumbnails")
...
class VideoPlayerWindow(Adw.ApplicationWindow):
    ...
    def __init__(self, **kwargs):
        ...
        self.set_title("Video Cast Player")
        ...
        # Cast Device Manager, HTTP-Server und Video-Converter
        self.cast_manager = CastDeviceManager()
    ...
    def setup_header_bar(self):
        ...
        # Cast device search button
        scan_button = Gtk.Button()
        scan_button.set_icon_name("network-wireless-symbolic")
        scan_button.set_tooltip_text("Search for Cast devices")
    ...
    def setup_sidebar(self):
        """Erstellt die Seitenleiste for Playlist and Cast"""
        ...
        # Cast device header
        label = Gtk.Label(label="Cast Devices")
        ...
        self.cast_expander = Gtk.Expander()
        self.cast_expander.set_label("Erweiterte Informationen")
        self.cast_expander.set_visible(False)  # Nur sichtbar wenn verbunden
        ...
        self.cast_expander.set_child(status_details_box)
        chromecast_section.append(self.cast_expander)
    ...
    def update_timeline(self):
        ...
        # Update Cast status display (nur im Cast mode)
        if self.play_mode == "chromecast":
    ...
    def perform_seek(self, position_seconds):
        ...
        if self.play_mode == "local":
            ...
            # Cast device seeking in Thread ausführen um UI nicht zu blockieren
            if self.cast_manager.mc:
                def chromecast_seek():
                    # For Cast devices we set wir das Flag nach einer kurzen Verzögerung zurück,
                ...
                print("Cannot seek: No Cast device connection")
    ...
    def setup_performance_optimizations(self):
        ...
        if self.play_mode == "local":
            ...
            self.cast_expander.set_visible(False)
    ...
    def setup_controls(self):
        ...
        self.mode_switch.set_tooltip_text("Zwischen lokaler Wiedergabe and Cast mode")
    ...
    def update_chromecast_status_display(self):
        """Aktualisiert die erweiterte Cast device status-Anzeige"""
        ...
        if status['connected']:
            # Zeige Expander
            self.cast_expander.set_visible(True)
    ...
    def on_connected(self, success, device_name):
        ...
        if success:
            ...
            # Wechsle automatisch in Cast mode nach erfolgreicher Verbindung
            ...
            print(f"Cast mode aktiviert nach Verbindung mit '{device_name}'")
            ...
            if self.current_video_path:
                ...
                # Start Cast streaming in Thread
                def start_streaming():
                    from pathlib import Path
                ...
                self.status_label.set_text(f"Cast device ready: {device_name}")
        else:
    ...
    def _update_mode_label(self):
        ...
        if self.play_mode == "chromecast":
            self.mode_label.set_markup('<span foreground="#4A90E2"><b>Cast</b></span>')
        else:
    ...
    def on_mode_changed(self, switch, gparam):
        """Wechselt zwischen lokalem und Cast mode"""
        if switch.get_active():
            # Wechsel zu Cast mode

            # Prüfe ob bereits searched for Cast devices wurde
            if not self.cast_manager._discovery_browser and not self.cast_manager.selected_cast:
                ...
                dialog = Adw.AlertDialog.new(
                    "Search for Cast Device",
                    "To use Cast zu können, müssen Sie zuerst nach verfügbaren Geräten suchen.\n\n"
                    "Klicken Sie on the Cast icon oben rechts, um nach Geräten zu suchen."
                )
            ...
            # Aktualisiere Play-Button auf "Play" since Cast streaming
            ...
            # Synchronize volume to Cast device
            ...
            # Wenn ein Video geladen ist und a Cast device connected ist,
            # starte automatisch das Streaming
            if self.current_video_path and self.cast_manager.selected_cast:
                self.status_label.set_text("Cast mode: Bereit")
                print("Cast mode aktiv. Drücke Play, um das Streaming zu starten.")
            elif self.current_video_path and not self.cast_manager.selected_cast:
                self.status_label.set_text("Cast mode - Kein Gerät verbunden")
            else:
                self.status_label.set_text("Cast mode")
        else:
            ...
            # Speichere aktuelle Cast device position falls Streaming aktiv
            chromecast_position = None
            if self.cast_manager.mc and self.cast_manager.selected_cast:
                print("Stop Cast streaming...")
                try:
                    # Hole aktuelle position from Cast device
                    chromecast_position = self.cast_manager.get_position()
                    if chromecast_position:
                        print(f"Cast device position gespeichert: {self.format_time(chromecast_position)}")

                    self.cast_manager.stop()
                    self.status_label.set_text("Cast stopped, Modus: Lokal")
                    # Erlaube wieder Standby
                    self.uninhibit_suspend()
                except Exception as e:
                    print(f"Fehler beim Stoppen von Cast: {e}")
            ...
            if self.current_video_path:
                ...
                # Wenn eine Cast device position vorhanden ist, springe dorthin
    ...
    def inhibit_suspend(self):
        """Verhindert Standby during Cast streaming"""
    ...
    def on_play(self, button):
        ...
        if self.play_mode == "local":
            ...
            if is_media_loaded:
                ...
                # Starte HTTP-Server und stream to Cast device
    ...
    def on_pause(self, button):
        ...
        if self.play_mode == "local":
            ...
            print(f"\n=== on_pause() aufgerufen im Cast mode ===")
            print(f"Cast device selected: {self.cast_manager.selected_cast is not None}")
    ...
    def _set_current_volume(self, volume):
        ...
        if self.play_mode == "local":
            ...
            # Cast device volume in Thread setzen um UI nicht zu blockieren
    ...
    def on_set_speed(self, action, param):
        ...
        # Nur für lokale Wiedergabe (Cast devices do not support variable Geschwindigkeit)
        if self.play_mode == "local":
    ...
    def on_show_about(self, button):
        ...
        about = Adw.AboutWindow(
            transient_for=self,
            application_name="Video Cast Player",
            application_icon="gnome-cast-player",
            developer_name="DaHool",
            version="2.0.1",
            developers=["DaHool"],
            copyright="© 2025 DaHool",
            license_type=Gtk.License.MIT_X11,
            website="https://github.com/berlinux2016/gnome-cast-player",
            issue_url="https://github.com/berlinux2016/gnome-cast-player/issues",
            comments="Ein moderner GTK4-Videoplayer mit Cast Device Support, der für eine nahtlose Wiedergabe sowohl lokal als auch on Cast device-Geräten optimiert ist. Inklusive Hardware-Beschleunigung für AMD und NVIDIA GPUs.\n\nMit Liebe für Simone programmiert ❤️"
        )
        ...
        about.add_credit_section(
            ...
            [
                ...
                "Cast streaming with HTTP-Server",
    ...
    def on_close_request(self, window):
        ...
        try:
            if self.cast_manager.selected_cast:
                print("Stoppe Cast playback...")
                self.cast_manager.stop()
                self.cast_manager.disconnect()
        except Exception as e:
            print(f"Fehler beim Trennen von Cast: {e}")
...
class VideoPlayerApp(Adw.Application):
    ...
    def __init__(self):
        super().__init__(application_id='org.gnome.CastPlayer')
