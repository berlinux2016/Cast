...
pkgname=gnome-cast-player
pkgver=2.0.1
pkgrel=1
pkgdesc="Modern GTK4 video player with Cast device streaming and hardware acceleration"
arch=('any')
url="https://github.com/berlinux2016/gnome-cast-player"
license=('MIT')
depends=(
    'python>=3.9'
    'python-gobject'
...
build() {
    ...
    for lang in locale/*/LC_MESSAGES/*.po; do
        if [ -f "$lang" ]; then
            ...
            msgfmt "$lang" -o "locale/${lang_code}/LC_MESSAGES/gnome-cast-player.mo"
        fi
    done
}
...
package() {
    ...
    elif [ -f gnome-cast-player.svg ]; then
        install -Dm644 gnome-cast-player.svg "${pkgdir}/usr/share/icons/hicolor/scalable/apps/${pkgname}.svg"
        install -Dm644 gnome-cast-player.svg "${pkgdir}/usr/share/icons/hicolor/scalable/apps/com.videocast.player.svg"
    fi

    # Install desktop file
    install -Dm644 gnome-cast-player.desktop "${pkgdir}/usr/share/applications/${pkgname}.desktop"
    ...
    if [ -f gnome-cast-player.appdata.xml ]; then
        install -Dm644 gnome-cast-player.appdata.xml "${pkgdir}/usr/share/metainfo/${pkgname}.appdata.xml"
    fi
